...
def tg_edit_keyboard(bot_token: str, chat_id: int, message_id: int, buttons: list) -> None:
    ...
    try:
        ...
        print(f'  ⚠️  editMessageReplyMarkup failed: {e}')
...
def am_download(track_id: str) -> Optional[str]:
    """
    Trigger an iTunes Match download and return the local POSIX path once done.
    Triggers the download via AppleScript, then polls Python-side for up to 4 min.
    ...
    try:
        ...
        return None
    # Step 1 — check current state and trigger download if needed
    trigger_script = (
        'tell application "Music"\n'
        '    set t to (first track of library playlist 1 whose database id is '
        + str(tid) + ')\n'
        '    try\n'
        '        if location of t is not missing value then\n'
        '            return "local|||" & POSIX path of (location of t)\n'
        '        end if\n'
        '    end try\n'
        '    try\n'
        '        download t\n'
    ...
    try:
        ...
        if raw.startswith('local|||'):
            path = raw[len('local|||'):]
            if path and Path(path).exists():
                print(f'  am_download [{track_id}]: already local → {path}')
                return path
            # path reported but file missing on disk — fall through to poll
            print(f'  am_download [{track_id}]: location returned but file absent, polling…')
        elif raw.startswith('downloading|||'):
            print(f'  am_download [{track_id}]: download triggered, beginning to poll…')
        else:
            print(f'  am_download [{track_id}]: unexpected response {raw!r}, proceeding to poll')
    except Exception as e:
        print(f'  am_download trigger error: {e}')
        return None

    # Step 2 — poll every 5 s for up to 4 minutes
    print(f'  am_download [{track_id}]: polling for file…')
    ...
    while time.time() < deadline:
        time.sleep(5)
        poll += 1
        path = _am_get_location(track_id)
        if path:
            print(f'  am_download [{track_id}]: file ready after {poll} polls → {path}')
            found_path = path
            break
    ...
    print(f'  am_download [{track_id}]: file never appeared after 240 s — track likely streams-only')
    return None
...
def handle_find_command(bot_token: str, chat_id: int, from_uid: int,
    ...
    if tracks:
        ...
        lines = [f'🎵 Found {len(tracks)} match{"es" if len(tracks) != 1 else ""} in Apple Music:\n']
        lines.append('_Tap tracks to select, then tap Done._')
        for i, t in enumerate(tracks, 1):
            lines.append(f'  {i}. *{t["title"]}* — {t["artist"]}  _{t["album"]}_{_cloud_badge(t)}')

        buttons = _am_multiselect_buttons(tracks, selected=[])
...
def _am_multiselect_buttons(tracks: list, selected: list) -> list:
    """Build toggle buttons for Apple Music multi-select."""
    buttons = []
    for i, t in enumerate(tracks):
        check = '✅' if i in selected else '☐'
        label = f'{check} {i + 1}. {t["title"][:45]}'
        buttons.append([(label, f'ams:{i}')])
...
def _handle_am_confirm_reply(bot_token: str, chat_id: int, from_uid: int,
    ...
    # Toggle a track: ams:<0-based-index>
    ...
    # Legacy single-pick fallback (e.g. '1', '2' from old pending state after restart)
    if cleaned == 'yes' and len(tracks) == 1:
        idx = 0
    elif cleaned.isdigit() and 1 <= int(cleaned) <= len(tracks):
        idx = int(cleaned) - 1
    else:
        return  # unrecognised — stay silent
    ...
    if cloud_status in _CLOUD_ONLY_STATUSES:
        ...
        tg_send(bot_token, chat_id,
                ...
                f'_(May take up to 2 minutes if the file needs to download)_')

    path = am_download(track['id'])
    if not path:
        tg_send(bot_token, chat_id,
